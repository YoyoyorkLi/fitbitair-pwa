...
// --- the SYNC_KEY path (external cron pinger) ------------------------------
{
  process.env.SYNC_KEY = "s".repeat(40);
  const calls = stubFetch({ userOk: false });   // no valid session at all
  const res = mockRes();
  await handler({ method: "POST", headers: { "x-sync-key": "s".repeat(40) } }, res);
  ok(res.code === 202 && dispatched(calls) && !calls.some((c) => c.url.includes("/auth/v1/user")),
    "matching x-sync-key -> 202, session check skipped");
}
{
  process.env.SYNC_KEY = "s".repeat(40);
  const calls = stubFetch({ userOk: false });
  const res = mockRes();
  await handler({ method: "POST", headers: { "x-sync-key": "wrong" } }, res);
  ok(res.code === 401 && !dispatched(calls), "wrong x-sync-key falls through to the session check -> 401");
  delete process.env.SYNC_KEY;
}
{
  const calls = stubFetch({ userOk: false });
  const res = mockRes();
  await handler({ method: "POST", headers: { "x-sync-key": "anything" } }, res);
  ok(res.code === 401 && !dispatched(calls), "x-sync-key with SYNC_KEY unset -> that path does not exist");
}
