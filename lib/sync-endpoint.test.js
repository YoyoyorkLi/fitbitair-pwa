...
}
{
  ...
  const res = mockRes();
  await handler(post, res);
  ok(res.code === 502 && /expired or revoked/.test(res.body.error), "expired token is reported as such");
}
{
  stubFetch({ dispatchStatus: 403 });
  const res = mockRes();
  await handler(post, res);
  ok(res.code === 502 && /Actions: write/.test(res.body.error), "missing permission names the permission");
}
